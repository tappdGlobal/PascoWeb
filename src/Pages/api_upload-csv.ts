...
import { sanitizeHeader, mapRowToRaw, cleanValue, toSnakeCaseKeyMap } from "../utils/csvMapping";

// Server-side only: use SERVICE ROLE KEY (DO NOT expose this on client)
...
// csv mapping helpers are imported from ../utils/csvMapping
...
export default async function handler(req: any, res: any) {
  ...
  // Require authenticated user (bearer token) to perform uploads
  const user = await verifyUserFromAuthHeader(req as any);
  if (!user) return res.status(401).json({ error: 'Unauthorized' });

  // Map each row to sanitized keys
  const mapped = rows.map(mapRowToRaw);
    ...
    function valueForCanonical(mappedRow: Record<string, any>, canonicalKey: string) {
      // If no mapping provided, try direct common keys
      if (!mapping) return mappedRow[canonicalKey] ?? mappedRow[canonicalKey.replace(/_/g, "_")];
      // Find original header that maps to this canonicalKey
    ...
    // Additionally, attempt to map raw rows into the "jobs" table so dashboards can read them directly.
    // This mapping is best-effort: we will map common column names where available.
    try {
      ...
      const job: any = {};
          ...
          job.services = [];
          job.activityLog = [];
          // parse numeric billing fields and map to camelCase keys expected by app
      ...
      // Before upserting, convert known camelCase keys to snake_case DB columns
      const jobsSnake = jobsToInsert.map(j => {
        const s = toSnakeCaseKeyMap(j);
        // ensure numeric fields are numbers in snake keys
